...
export default function SargaView() {
  const { kandamId, sargaId } = useParams();
  ...
  const sargaIndex = kandam?.sargas.findIndex(s => s.id === sargaId) ?? -1;
  const sarga = kandam?.sargas[sargaIndex];

  if (!kandam || !sarga) return <div>சர்க்கம் காணப்படவில்லை</div>;

  const prevSarga = sargaIndex > 0 ? kandam.sargas[sargaIndex - 1] : null;
  const nextSarga = sargaIndex < kandam.sargas.length - 1 ? kandam.sargas[sargaIndex + 1] : null;

  const scrollToParagraph = (idx: number) => {
    if (idx >= 0 && idx < sarga.content.length) {
    ...
    <>
      <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20 relative">
        <div className="max-w-3xl mx-auto mb-4">
          <Breadcrumbs items={[
            ...
            { label: kandam.name, href: `/kandams/${kandam.id}` },
            { label: sarga.title }
          ]} />
        </div>
      ...
      >
        ...
        <div className="max-w-3xl mx-auto flex items-center justify-between relative z-10">
          <div className="w-10 sm:w-12 flex justify-start">
            {prevSarga && (
              <Link to={`/kandams/${kandam.id}/sarga/${prevSarga.id}`} className="p-2 -ml-2 rounded-full hover:bg-[var(--accent-soft)] transition-colors" style={{ color: 'var(--accent-color)' }}>
                <ChevronLeft size={24} />
              </Link>
            )}
          </div>
          <div className="text-center flex-1 px-4">
            <div className="font-bold mb-1 tracking-widest uppercase text-[14px] sm:text-[16px]" style={{ color: 'var(--accent-color)' }}>
              {kandam.name} - சர்க்கம் {sarga.number}
            </div>
          ...
          <div className="w-10 sm:w-12 flex justify-end">
            {nextSarga && (
              <Link to={`/kandams/${kandam.id}/sarga/${nextSarga.id}`} className="p-2 -mr-2 rounded-full hover:bg-[var(--accent-soft)] transition-colors" style={{ color: 'var(--accent-color)' }}>
      ...
      >
        <div className={`${getFontSizeClass(fontSize)} space-y-6 leading-[2.2] opacity-90 relative`}>
          {sarga.content.map((paragraph, idx) => (
        ...
        <nav className="mt-16 pt-8 border-t flex flex-col sm:flex-row items-center justify-between gap-4" style={{ borderColor: 'var(--border-color)' }}>
          {prevSarga ? (
            <Link
              to={`/kandams/${kandam.id}/sarga/${prevSarga.id}`}
            ...
            >
              ...
              <div className="flex-1 text-left">
                <div className={`${getFontSizeClass(fontSize, 'ui')} opacity-50 uppercase tracking-wider font-semibold mb-1`}>முந்தைய சர்க்கம்</div>
                <div className={`${getFontSizeClass(fontSize, 'subtext')} font-medium line-clamp-1`}>{prevSarga.title}</div>
              </div>
            </Link>
          ) : (
          ...
          {nextSarga ? (
            <Link
              to={`/kandams/${kandam.id}/sarga/${nextSarga.id}`}
            ...
            >
              <div className="flex-1">
                <div className={`${getFontSizeClass(fontSize, 'ui')} opacity-50 uppercase tracking-wider font-semibold mb-1`}>அடுத்த சர்க்கம்</div>
                <div className={`${getFontSizeClass(fontSize, 'subtext')} font-medium line-clamp-1`}>{nextSarga.title}</div>
              </div>
            ...
            <div className={`w-full sm:flex-1 text-center sm:text-right py-4 ${getFontSizeClass(fontSize, 'subtext')}`}>
              <Link to={`/kandams/${kandam.id}`} className="font-medium hover:underline" style={{ color: 'var(--accent-color)' }}>
      ...
      >
        <div className="flex flex-col items-center gap-1.5 px-2 py-3 rounded-full border shadow-lg backdrop-blur-md pointer-events-auto" style={{ backgroundColor: 'var(--card-bg)', opacity: 0.95, borderColor: 'var(--border-color)', color: 'var(--text-color)' }}>
          ...
          <button 
            disabled={currentParagraph === sarga.content.length - 1} 
            onClick={() => scrollToParagraph(currentParagraph + 1)}
            className="p-1.5 lg:p-2 rounded-full hover:bg-[var(--accent-soft)] disabled:opacity-30 disabled:hover:bg-transparent transition-colors flex items-center justify-center"
            style={{ color: 'var(--accent-color)' }}
